<script lang="ts">
	...
	export let month = 'January';
	export let year = '1977';
	export let Pattern: ConstructorOfATypedSvelteComponent;
</script>

<div
	class={`mx-0 flex h-[9in] w-[4in] flex-wrap rounded-sm border-2 border-gray-400 bg-white pt-1.5 text-${color}`}
>
	<div>
		<div class="flex h-fit">
			<div class="flex w-1/3 px-2" />
			<div class="w-2/3">
				<div class={`flex border-b-2 border-${color} text-xs`}>
					<div class="py-0.5">
						<p>United States</p>
				...
				<div class={`border-b-2 border-${color} py-0.5 text-xs`}>
					<p>{shortTitle}</p>
				</div>
			</div>
		</div>
		<div class="flex h-fit">
			<div class="w-1/3 py-1.5 px-5">
				<svg
					class="mx-auto w-10/12 align-top"
			...
			<div class="w-2/3">
				<div class="pr-3 text-start text-3xl font-semibold">
					<h1>{title}</h1>
				</div>
				<div class="mt-6 pr-2 text-start text-3xl">
					<h1>{description}</h1>
				</div>
			</div>
		</div>
	...
	<div class="w-full">
		<svelte:component this={Pattern} {color} />
	</div>
</div>
